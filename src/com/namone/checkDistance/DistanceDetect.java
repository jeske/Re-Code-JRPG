...
public class DistanceDetect {
	...
	// ABLE TO PASS IN ANY PLAYER/ENEMY (AND IT'S RANGE, POSITION, ETC.) TO CHECK
	...
	public boolean checkPlayerAttackRange(Player player, ArrayList<Enemy> Enemy, int maxEnemy) {	
		
		/*
		 * FOR EACH ENEMY IN THE ARRAY LIST CHECK IF IS IN RANGE... THEN MAKE
		 * A DECISION
		 * 
		 *  - ALMOST WORKING -
		 *  
		...
		for (int i = 0; i <= maxEnemy; i++){
			...
			System.out.println("ENEMY # : " + i + " - X : " + Enemy.get(i).enemyX
					+ " - Y : " + Enemy.get(i).enemyY);
			// TESTING PURPOSES
			...
			if(playerDeltaX <= player.playerRange && playerDeltaX >= -player.playerRange) {
				inRange = true;
			}
			
			else if(playerDeltaY >= player.playerRange && playerDeltaY <= -player.playerRange) {
				inRange = true;
			}
